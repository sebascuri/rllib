...
policy = NNPolicy(env.dim_state, env.dim_action, env.num_states, env.num_actions,
                  squashed_output=True, layers=[64, 64], tau=0.02,
                  action_scale=env.action_scale, biased_head=False)
zero_bias(policy)

policy = FelixPolicy(env.dim_state, env.dim_action, deterministic=False,
...
agent = MPPOAgent(ENVIRONMENT, policy=policy, q_function=q_function,
                  criterion=nn.MSELoss,
                  optimizer=optimizer, memory=memory,
                  epsilon=EPSILON, epsilon_mean=EPSILON_MEAN, epsilon_var=EPSILON_VAR,
                  num_action_samples=NUM_ACTION_SAMPLES,  train_frequency=0,
                  num_rollouts=NUM_ROLLOUTS, target_update_frequency=4,
