...
for h in plan_horizon:
    ...
    axes[0].fill_between(np.arange(len(mean)), mean-std, mean+std, alpha=0.1)

for h in plan_horizon:
    mean = df[df.plan_horizon == h].model_return.mean(level=0)
    std = df[df.plan_horizon == h].model_return.std(level=0).fillna(0)
    axes[1].plot(mean, label=f"Plan Horizon {h}")
    axes[1].fill_between(np.arange(len(mean)), mean - std, mean + std, alpha=0.1)
