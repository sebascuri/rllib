...
class MPCPolicy(AbstractPolicy):
    ...
    def __init__(self, dynamical_model, reward_model, horizon, terminal_reward=None,
                 termination=None, gamma=1, num_samples=400, num_iter=5,
                 num_elites=None, solver='CEM', warm_start=False):
        super().__init__(dynamical_model.dim_state, dynamical_model.dim_action)
    ...
    def forward(self, state):
        ...
        if self.solver.lower() == 'cem':
            actions = cem_shooting(
                self.dynamical_model, self.reward_model, self.horizon, state,
