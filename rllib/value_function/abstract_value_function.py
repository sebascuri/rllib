...
class AbstractQFunction(AbstractValueFunction):
    ...
    @abstractmethod
    def max(self, state):
        """Return the maximum value (over actions) of the Q-Function at a given state.
        ...
        ------
        NotImplementedError:
            If maximization is not solvable in closed form.
        """
        raise NotImplementedError

    @abstractmethod
    def argmax(self, state):
        """Return the action that maximizes of the Q-Function at a given state.
    ...
    def extract_policy(self, temperature=1.):
        """Return the policy induced by the Q-Function.

        Parameters
        ----------
        temperature: float, optional
            Temperature that scales the variance of the output policy.

        Returns
        -------
        policy: rllib.policy.AbstractPolicy
