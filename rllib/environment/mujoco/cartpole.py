...
class CartPoleEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def step(self, action: np.ndarray):
        ...
        end_effector = self._get_end_effector_pos(ob) - np.array([0., pendulum_length])

        reward_dist = np.exp(-np.sum(np.square(end_effector)) / (pendulum_length ** 2))
    ...
    def _get_end_effector_pos(self, x):
        ...
        pendulum_length = self.pendulum_length
        return np.array([x0 - pendulum_length * sin, -pendulum_length * cos])
