...
def discount_cumsum(rewards: Array, gamma: float = 1.0,
                    reward_transformer: RewardTransformer = RewardTransformer(),
                    ) -> Array: ...


def discount_sum(rewards: Tensor, gamma: float = 1.0, reward_transformer: RewardTransformer = RewardTransformer(),
                 ) -> Array: ...


def mc_return(trajectory: Trajectory, gamma: float = 1.0,
              value_function: AbstractValueFunction = None, entropy_reg: float = 0.,
              reward_transformer: RewardTransformer = RewardTransformer()) -> Tensor: ...


def mb_return(state: State, dynamical_model: AbstractModel,
